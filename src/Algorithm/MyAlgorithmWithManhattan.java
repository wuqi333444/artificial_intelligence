...
public class MyAlgorithmWithManhattan {
	World world;
	...
	int nodeExpand = 0;
	long time=0;
	public MyAlgorithmWithManhattan(World world){
		this.world = world;
	}
	class Cell{
		Cell parent;
    ...
    private void countH(Cell cell,Cell endCell){
    	double d = distanceToTime((Math.abs(cell.self.x-endCell.self.x)+Math.abs(cell.self.y-endCell.self.y)));
        cell.setH(d);
    }
    //计算F值
    ...
    public String getPerformance(){
    	return "MyalgorithmWithManhattan(RunningTime:"+time+"   Totalcost:"+getCost()+"   NodeExpand:"+nodeExpand+")";
    }
}
