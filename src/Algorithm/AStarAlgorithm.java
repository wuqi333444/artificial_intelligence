...
import Algorithm.AStarAlgorithmWithDirectDistance.Cell;
import Element.Node;
...
public class AStarAlgorithm {
	World world;
	long time=0;
	public int nodeExpand = 0;
	List<Node> result = new ArrayList<Node>();
	List<Cell> resultList = new ArrayList<Cell>();
	public AStarAlgorithm(World world){
		this.world = world;
	}
	class Cell{
		Cell parent;
    ...
    private void countH(Cell cell,Cell endCell){
        cell.setH((Math.abs(cell.self.x-endCell.self.x)+Math.abs(cell.self.y-endCell.self.y)));
    }
    //计算F值
    ...
    public double waitTime(Node n,Cell Parentcell,double cost){
    	if (Parentcell==null||Parentcell.getParent()==null){//have not turned
    	 ...
    	 public String getPerformance(){
    	    	return "A*algorithmWithManhattan(RunningTime:"+time+"   Totalcost:"+getCost()+"   NodeExpand:"+nodeExpand+")";
    	    }
}
